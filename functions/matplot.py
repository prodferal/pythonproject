...
def basic_statistics_plot(data):
    ...
    ax1.bar(x_1, y_1, color=colors, width=0.65)
    ax1.set_title('Text Composition')
    ax1.set_ylabel('Count')
    ax1.set_ylim(0, max(y_1) * 1.15)
...
def word_analysis_plot(data):
    ...
    pairs_cwd = list(d.items())
    items_cwd = [[count, word] for (word, count) in pairs_cwd]
    items_cwd.sort(reverse = True) # Sort pairs in items
    top = items_cwd[:10]
    
    x_1 = np.array([w for _, w in top])
    y_1 = np.array([c for c, _ in top])

    ax1.bar(x_1, y_1, edgecolor='black', width=0.65)
    ...
    # Plot 2, another bar
    wdh = data['word_data']['word_hist']
    lengths = sorted(wdh.keys())
    counts_len = [wdh[i] for i in lengths]
    labels_len = [str(i) for i in lengths]

    ax2.bar(labels_len, counts_len, color='salmon', edgecolor='black', width=0.65)
    ax2.set_xlabel('Word Length (characters)')
    ax2.set_ylabel('Frequency')
    ax2.set_title('Word Length Distribution')
    plt.setp(ax2.get_xticklabels(), rotation=25, fontsize=8)
